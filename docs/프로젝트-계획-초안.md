...
- **빌드 에이전트**: Microsoft-hosted `windows-latest` 또는 self-hosted 에이전트
- **아티팩트**: `DACPAC`(스키마 상태), 필요 시 `BACPAC`(스키마+데이터)
- **배포 도구**: `SqlPackage.exe` / `SqlAzureDacpacDeployment` 태스크
- **인증**: Azure AD 서비스 주체(Service Principal) 또는 관리 ID(Managed Identity) + Key Vault
...
**CI(빌드): SSDT 프로젝트 → DACPAC 생성**
```yaml
trigger:
  branches: { include: [ main ] }
  paths: { include: [ 'database/**' ] }

pool: { vmImage: 'windows-latest' }

steps:
  - task: VSBuild@1
    displayName: 'Build SSDT (.sqlproj) → DACPAC'
    inputs:
      solution: 'database/**/*.sqlproj'
      configuration: 'Release'

  # (선택) tSQLt 단위 테스트 / SQL 정적 분석 실행 단계 삽입 지점
  ...
  - task: PublishBuildArtifacts@1
    inputs:
      ...
      ArtifactName: 'db-dacpac'
```

**CD(배포): DACPAC → SQL MI (서비스 주체 인증)**
```yaml
stages:
  - stage: Deploy_Test
    jobs:
      - deployment: DeployDB
        environment: 'SQLMI-Test'   # 승인 게이트를 Environment에 구성
        strategy:
          runOnce:
            deploy:
              steps:
                - task: SqlAzureDacpacDeployment@1
                  inputs:
                    azureSubscription: 'svc-connection-sqlmi'  # Service Principal
                    AuthenticationType: 'servicePrincipal'
                    ServerName: 'sqlmi-xxxx.database.windows.net'
                    DatabaseName: 'AppDb'
                    DacpacFile: '$(Pipeline.Workspace)/db-dacpac/AppDb.dacpac'
                    AdditionalArguments: >-
...
- **시크릿 관리**: 자격증명 하드코딩 금지 → **Azure Key Vault + Service Principal/Managed Identity**
- **환경별 승인 게이트**: Azure DevOps *Environments*에 프로덕션 승인자 지정
- **네트워크**: SQL MI는 VNet 내부 → **self-hosted 에이전트** 또는 프라이빗 엔드포인트 필요
...
| 위험 | 영향 | 대응 |
|------|------|------|
| SQL MI VNet 격리로 에이전트 접근 불가 | 배포 실패 | self-hosted 에이전트/프라이빗 엔드포인트 |
