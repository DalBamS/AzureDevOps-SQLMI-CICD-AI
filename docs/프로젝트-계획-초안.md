...
- **인스턴스 오브젝트**: DACPAC 성공 후 별도 job에서 공용 token parser와 strict SQLCMD 값
  검사, mutation 대상과 catalog/name이 일치하는 명시적 guard allowlist를 통과한 멱등 Entra
  login/SQL Agent job 적용
...
승인본과 비교하고, 동일 Script의 검증된 sanitized SQL을 `Invoke-Sqlcmd -Query`로 메모리에서
실행합니다. 파이프라인은
